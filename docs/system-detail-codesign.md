# 视频生成 Prompt 系统架构草案（V2 + V3 详设）

> 基于"轻 PageIndex（离线）、重 LLM"的混合推理架构，支持多模型与多模态样例。**检索层依赖 PageIndex 离线版本，PageIndex 的模型也使用 Kimi。**  
> 高层设计：`system-design-v1.md`（草案）、`system-design-v2.md`（MVP）、`system-design-v3.md`（多角色协作）。
...
- **PageIndex 离线版退居辅助**：PageIndex 离线版本仅作为"精确技术参数缓存层"，用于存储频繁更新的禁忌词、精确数值（焦距/色温/物理公式）和私有知识库
- **分层解耦**：将"创意理解"（LLM）与"技术约束"（PageIndex 离线版 + Rule-based）分离，避免 PageIndex 误召回破坏创意意图
...
│    "needs_precise_params": false   // 是否需要查 PageIndex   │
│  }                                                          │
└──────────────────────┬──────────────────────────────────────┘
                       │
          ┌────────────┴────────────┐
...
| 功能模块 | 技术选型 | 理由 |
|---------|---------|------|
| **意图解析** | LLM 直接推理 | 模糊语义理解必须依赖 LLM 的联想能力，检索无法处理"老电影的感觉"这类隐喻 |
| **风格映射** | LLM + Few-shot | 让 LLM 参考 3-5 个示例学习风格，比向量匹配更精准 |
| **技术参数** | PageIndex 离线版 | 镜头焦距、色温值等**精确数值**用 PageIndex 离线版存储，避免 LLM hallucinate；PageIndex 模型使用 Kimi |
| **物理检查** | LLM 逻辑推理 | 必须验证"动量守恒"、"流体连续性"等物理逻辑，检索无法推理 |
| **禁忌词过滤** | PageIndex 离线版 (频繁更新) | Seedance2 的禁用词列表每周更新，PageIndex 比模型微调成本低 |
...
**设计原则**: Skill 不是"检索模板"，而是"结构化约束 + 多模态示例库 + 可选多角色配置 (V3)"

```yaml
skills/
├── role_loader.py                # V3: 从各 skill 加载 roles.yaml
├── seedance2/                    # 针对 Seedance2 的 Skill 包
│   ├── roles.yaml                # V3: 多角色协作配置 (见 3.5 节)
...
### 3.3 技术参数缓存层 (tech-cache/) —— 唯一的 PageIndex 离线版模块

**定位**: 仅存储**事实性、精确性、频繁更新**的数据；**依赖 PageIndex 离线版本，PageIndex 的模型使用 Kimi。**

```yaml
tech-cache/
├── embeddings/               # 向量存储 (仅此类数据，由 PageIndex 离线版管理)
...
**触发条件** (严格控制):
```python
def should_use_page_index(intention: IntentionObject) -> bool:
    """仅在以下情况触发 PageIndex 离线版"""
    return (
...
└── local_rag/             # 私有 PageIndex 数据 (客户品牌规范等，由 PageIndex 离线版索引)
    └── client_brand_guidelines.md
```

### 3.5 多角色协作 (V3，参见 system-design-v3.md)

**用途**: 在 Skill 内配置多个「角色」，按顺序参与生成，通过角色互动产出最终 Prompt。

**配置**: 各 Skill 目录下可选 `roles.yaml`：

```yaml
# src/skills/seedance2/roles.yaml 示例
description: "Seedance2 多角色协作：导演 → 摄影 → 成稿"
roles:
  - id: director
    name: 导演
    order: 1
    system_prompt: |
      你是视频创意导演。根据用户意图提炼：主题与情绪、节奏与时长...
  - id: cinematographer
    name: 摄影
    order: 2
    system_prompt: |
      你是摄影指导。根据导演的创意要点，设计镜头语言...
  - id: finalizer
    name: 成稿
    order: 3
    system_prompt: |
      你是成稿师。将前述要点整合为一条完整的、可直接用于视频生成的 Prompt...
```

**加载**: `role_loader.load_skill_roles(skill_name)` 读取 `src/skills/{skill_name}/roles.yaml`，解析 `roles` 列表并按 `order` 排序；若文件不存在或解析失败返回空列表，则退化为 V2 单轮生成。

**流水线**: `PromptGenerator` 若检测到非空角色列表，则对每个角色依次调用 `client.chat(role.system_prompt, user_content)`，其中 `user_content` = 用户意图 JSON + 前序所有角色的输出；最后一名的输出作为 `prompt_text`。

**LLM 插件扩展**: `KimiClient`（及后续实现）提供 `chat(system_prompt, user_content) -> str`，用于单轮对话，供多角色流水线使用。

**运行时可配置**: 环境变量 `SKILL_NAME` 指定当前 Skill（默认 `seedance2`）；主入口启动时打印「已加载 N 个角色: [name1, name2, ...]」，结束时若有 `role_outputs` 则逐条打印各角色输出，再打印「最终 Prompt」。

**输出约定**: `RunResult.role_outputs: list[(role_name, content)]`，多角色时有值，单轮时为空列表。
...
- **Path B (PageIndex 离线版)**: 查询 "85mm 镜头在 Full Frame 上的视角范围" (精确数值)
...
### ADR-001: 为什么意图解析不用 PageIndex（检索）？
- **决策**: 意图解析必须直接使用 LLM 推理
- **理由**: 用户输入"像老电影的感觉"是隐喻，检索的向量匹配会字面化理解为"old movie"，而 LLM 能推理出"16mm 胶片、色偏、轻微抖动"等隐含要素
- **影响**: 增加 LLM Token 消耗，但大幅提升准确性

### ADR-002: PageIndex 离线版仅用于技术参数缓存
- **决策**: PageIndex 离线版本退居"精确数值查询"角色；**PageIndex 的模型使用 Kimi。**
- **理由**: 
  - LLM 可能 hallucinate 镜头焦距 (如记错 50mm 在 APS-C 上的等效焦距)
  - 视频模型禁忌词列表需频繁更新，PageIndex 离线版比模型微调成本低
- **边界**: 禁止用 PageIndex 做风格匹配或创意生成
...
### ADR-007: Skill 多角色协作 (V3)
- **决策**: 在 Skill 内通过 `roles.yaml` 配置多个角色，按 `order` 顺序依次调用 LLM，每角色可见「意图 + 前序输出」，最后一名产出最终 Prompt
- **理由**:
  - 单一大模型单轮生成易遗漏专业视角（如导演创意 vs 摄影语言 vs 成稿规范）
  - 多角色流水线可显式分工（创意 → 镜头 → 成稿），便于调试与扩展
  - 配置驱动、无 roles 时退回 V2 单轮，兼容现有行为
- **实现**: `role_loader.load_skill_roles(skill_name)`、`client.chat(system, user)`、`PromptResult.role_outputs`、`SKILL_NAME` 环境变量；详见 `system-design-v3.md` 与本文 3.5 节
...
│   ├── skills/
│   │   ├── role_loader.py      # V3: 加载各 skill 的 roles.yaml
│   │   ├── seedance2/          # 特定模型 Skill
│   │   │   ├── roles.yaml      # V3: 多角色配置 (可选)
│   │   │   ├── templates/      # Jinja2 模板
│   │   │   ├── constraints/    # 分层约束
│   │   │   └── examples/       # 多模态样例库
│   │   │       ├── index.yaml  # 轻量索引
...
│   │       ├── physics_rules.yaml
│   │       └── camera_lexicon.yaml
│   └── tech_cache/             # 技术参数缓存 (唯一 PageIndex 离线版区)
│       ├── indexer.py
│       └── retriever.py
...
### Phase 1 (MVP) 与 V3 增强
- 实现 Layer 1 (意图解析) + Layer 3 (生成)，**完全不用 PageIndex**，验证 LLM 直接推理的准确性
- 硬编码常见技术参数作为 Few-shot
- **V3**：Skill 多角色协作（`roles.yaml` + `role_loader` + 顺序 chat 流水线），可选；无配置时退化为单轮生成

### Phase 2 (增强)
- 引入 tech-cache（**PageIndex 离线版**，模型使用 Kimi），仅用于：
  - 频繁更新的禁忌词列表
  - 精确镜头参数查询
- 实现约束分层加载与多模态样例库
...
- 多模态输入增强: 支持上传参考图，LLM 分析 + 可选 PageIndex 离线版匹配相似风格参数
...
**总结**: 本架构的核心是**"让大模型做它擅长的事（理解、推理、创造），让 PageIndex 离线版做它擅长的事（存储精确事实）"**；检索层依赖 PageIndex 离线版本，PageIndex 的模型使用 Kimi。通过模型中立设计和多模态样例库，构建可演进的视频 Prompt 生成系统。
```